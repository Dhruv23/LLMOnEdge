#!/usr/bin/env python3
"""
Combined Measurement Script
Runs the contexts defined in the plan (from measure_all.py),
and then automatically runs the high-latency buckets from measure_20k.py.
Outputs all metrics (including CPU and RAM usage) to a single CSV.
"""
...
def main():
    ...
    with open(args.out, file_mode, newline="") as csvfile:
        ...
        try:
            ...
            # --- PART 2: MEASURE 20k (HARDCODED) ---
            TARGET_CONTEXTS = [16250, 17500, 18750, 30000]
            ITERATIONS_PER_CTX = 10
            print(f"\n[INFO] Starting Extended Stress Test ({len(TARGET_CONTEXTS)} buckets x {ITERATIONS_PER_CTX} points)...")
